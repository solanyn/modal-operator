...
# Function calling controller with authentication
class ModalFunctionController:
    """Controller for calling Modal functions from local services."""

    def __init__(self):
        self.k8s_client = client.ApiClient()

    async def call_function(
        self, namespace: str, function_name: str, payload: Dict[str, Any], auth_token: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    f"{proxy_url}/modal-function/{function_name}", json=payload, headers=headers
                ) as response:
