...
class ModalPlaceholder:
    ...
    def _detect_workload_type(self):
        ...
        if any(port in str(self.original_command + self.original_args) for port in ["serve", "server", "api", "8080", "5000"]):
            return "function"
    ...
    async def run_as_job(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.modal_proxy_url}/modal-job",
                json=job_spec
            ) as response:
    ...
    async def run_as_function(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.modal_proxy_url}/modal-function",
                json=function_spec
            ) as response:
                result = await response.json()
    ...
    async def _start_proxy_server(self, function_url):
        ...
        async def proxy_handler(request):
            ...
            try:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        f"{self.modal_proxy_url}/modal-function/{os.getenv('POD_NAME', 'default')}",
                        json=data
                    ) as response:
                        result = await response.json()
